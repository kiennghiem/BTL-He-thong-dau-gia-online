...
public class AuctionService {
    ...
    /**
     * Cancels an auction, updates real-time state, and persists to DB.
     */
    public void cancelAuction(String auctionId) throws AuctionNotFoundException {
        ...
        persistenceExecutor.submit(() -> {
            try {
                ...
                if (updated) {
                    logger.info("Auction {} canceled in DB.", auctionId);
                }
            } catch (SQLException e) {
                logger.error("Error canceling auction status in DB: {}", e.getMessage(), e);
            }
        });
    }
        ...
        if (auction != null) {
            persistenceExecutor.submit(() -> {
                try {
                    ...
                    if (updated) {
                        logger.info("Auction {} finalized in DB.", auctionId);
                    }
                } catch (SQLException e) {
                    logger.error("Error finalizing auction status in DB: {}", e.getMessage(), e);
                }
            });
        }
    }
    ...
    public void placeBid(String auctionId, String bidderId, BigDecimal amount)
        ...
        persistenceExecutor.submit(() -> {
            ...
            if (!saved) {
                logger.error("CRITICAL: Bid for {} on {} failed to save to DB via BidDAO!", bidderId, auctionId);
            }

            try {
                boolean auctionUpdated = auctionDAO.placeBid(auctionId, bidderId, amount);
                if (!auctionUpdated) {
                    logger.error("CRITICAL: Auction atomic bid constraints rejected the value for: {}", auctionId);
                }
            } catch (SQLException e) {
                logger.error("CRITICAL: Database error updating auction current balance: {}", e.getMessage(), e);
            }
        });
    }
    ...
    public boolean createAuction(ItemType type, String name, String desc, BigDecimal startingPrice,
        ...
        if (itemSaved) {
            try {
                ...
                logger.error("Failed to insert auction entity mapping: {}", e.getMessage(), e);
            }
        }
        ...
        if (itemSaved && auctionSaved) {
            ...
            logger.info("New auction created: {}", auction.getId());
            return true;
        }
    ...
    public void loadActiveAuctions() {
        ...
        try {
            ...
            logger.info("Successfully loaded {} auctions into memory.", activeAuctions.size());
        } catch (SQLException e) {
            logger.error("Failed to load active auctions from DB: {}", e.getMessage(), e);
        }
    }
